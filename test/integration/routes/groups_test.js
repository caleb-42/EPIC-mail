...
describe('GROUPS API ENDPOINTS', () => {
  ...
  describe('Add user to group api/v1/groups/:id/users', () => {
    ...
    it('should not add user to a group if new group user ID is not valid', async () => {
      const res = await request(server).post('/api/v1/auth/signup').send(user1);
      const { token } = res.body.data[0];
      const resp = await request(server).post('/api/v1/groups/1/users').send({ email: 'john@epicmail.com' }).set('x-auth-token', token);
    ...
    it('should not add user to a group if new user is already in group', async () => {
      ...
      const { token } = res.body.data[0];
      await request(server).post('/api/v1/groups/').send({ name: 'caleb' }).set('x-auth-token', token);
      await request(server).post('/api/v1/groups/1/users').send({ email: 'sam@epicmail.com' }).set('x-auth-token', token);
      const resp = await request(server).post('/api/v1/groups/1/users').send({ email: 'sam@epicmail.com' }).set('x-auth-token', token);
    ...
    it('should not add admin user of group to a group', async () => {
      ...
      const { token } = res.body.data[0];
      await request(server).post('/api/v1/groups/').send({ name: 'caleb' }).set('x-auth-token', token);
      await request(server).post('/api/v1/groups/1/users').send({ email: 'ewere@epicmail.com' }).set('x-auth-token', token);
      const resp = await request(server).post('/api/v1/groups/1/users').send({ email: 'ewere@epicmail.com' }).set('x-auth-token', token);
